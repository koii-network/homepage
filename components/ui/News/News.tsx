...
export const Newsblock = ({ cover, title, content, logo, url, urltext, width }: NewsProps) => (
    <div className="px-6 md:max-w-sm pt-12">
        ...
        <Image className="inline" src={logo} alt="news logo" width="66" height="66"/>
        </div>
        } 
        <div className="mt-2 uppercase text-base leading-6">{title}</div>
        <div className="mt-2 text-sm leading-6">{content}</div>
        <a className="underline underline-offset-2 text-sm leading-6" href={url} rel="noopener noreferrer" target="_blank">{urltext}</a>
    </div>
